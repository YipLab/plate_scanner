    ...
    if running:
        ...
        if repeat.isdigit():
            ...
            for col in range(8):
                ...
                if (col < 7 and running == True): #Move to next row
                    sendSerial(xy_stage, "0pr"+str(well_size+adj_x)+";1pr"+str(adj_y+0.7*(-1)**(col))+";\r\n");
                    time.sleep(com_sleep);
                    print(recSerial(xy_stage));
                ...
                if well[0] in cols and well[1:].isdigit():
                    ...
                    if (row <= 12 and row >= 0):
                        if not orient:
                            col = 7-col;
                            row = 11-row
                        sendSerial(xy_stage, "0pr"+str((well_size+adj_x)*col)+";1pr"+str(well_size*(row+int(col%2==1))+adj_y*col+1.0*int(col%2==1))+";\r\n");
                        print(recSerial(xy_stage));
                        initCamera(camerafiles[0]);
                        print('scan plate')
                        sendSerial(xy_stage, "0lo1;\r\n");
